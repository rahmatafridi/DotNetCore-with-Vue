<template>
  <div>
    ...
    <CRow>
      <div>
        <CButton color="primary" @click="getData"
          >Get Weather Data</CButton
        >
        <CButton color="secondary" @click="PostData">Add Data</CButton>
      </div>
      <br />
      <CTable align="middle"
              class="mb-4 border"
...
export default {
  ...
  methods: {
    getData: function () {
      ...
      //  debugger
        root.tableExample = response.data
      })
    },
    addData: function () {
      ...
      let root = this
      axios({
        url: '/WeatherForecast/AddData',
        data: root.tableExample,
        method: 'POST'
      }).then((response) => {
      ...
      //axios.defaults.headers.common['Authorization'] =
      //  'Bearer ' + localStorage.getItem('token')
      //axios.post('/WeatherForecast/AddData', root.tableExample).then((response) => {
